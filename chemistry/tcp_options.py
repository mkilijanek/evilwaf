...
class TCPOptionsManipulator:
    ...
    window_scales: List[int] = field(
        default_factory=lambda: [8, 7, 6, 5, 4, 3, 2, 1, 0]
    )
    ...
    def _build_profile(
        ...
        return {"options": opts, "window": window}
    ...
    def windows_11_profile(self) -> Dict:
        return self._build_profile(
            mss=1460, sack=True, wscale=8, ts=None, nop_count=1, window=64240
        )
    ...
    def _all_profiles(self) -> Dict[str, Callable[[], Dict]]:
        return {
    ...
    def send_syn(
        self, dst_ip: str, dst_port: int, profile_name: Optional[str] = None
    ) -> Optional[object]:
